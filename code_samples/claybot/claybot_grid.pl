...
#	
#		This script updates the Grid Sheet.
#
...
# CONFIG
my $PUB_ID = $ARGV[0] || die "No pub id!";
...
# Read in the data
open(GRID, "<$DATADIR/GRID.txt")
	or die "Can't open GRID.txt: $!";
...
my $dbh = DBI->connect('dbi:mysql:XXX', 'XXX', 'XXXX', 
...
$dbh->do("DELETE FROM GRIDS WHERE PUB_ID=$PUB_ID");
...
while (<GRID>) { 
	...
	my $st = "INSERT INTO GRIDS ( " . join(", ", keys %DATA) .
			  "	) VALUES (" . $placeholders . ")";
